...
let tp_const = function
    | BoolV _ -> BoolT
    | IntV _ -> IntT
    | UndefV -> failwith "Impossible de déterminer le type d'une valeur non définie";;
...
let search_tp_funbind v env = 
    let rec aux = function
        | (a, FPdecl(tp,name,l))::c -> if v = a then FPdecl(tp,name,l) else aux c
        | _ -> raise NotFound
in aux env;; 
...
let type_creation = function 
    | FPdecl(t_retour, _, (Vardecl(a, b)::c)) -> 
        let rec aux = function 
            | [] -> b
            | [Vardecl(_, x)] -> FunT(b,x)
    		| Vardecl(_, x)::c -> FunT(x, aux c) 
    in FunT(aux c, t_retour)
    | FPdecl(t_retour, _, _) -> t_retour;;

(* type a variable within env *)
let tp_var env v = try (search_tp_localvar v env.localvar) with | NotFound -> try type_creation (search_tp_funbind v env.funbind) with | NotFound -> failwith "echec de typage, la variable n'est pas dans l'environnement";;
...
let convert_vardecl_list (vardecl_list : vardecl list) : (vname * tp) list =
    List.map (fun (Vardecl (vname, tp)) -> (vname, tp)) vardecl_list ;;

(* check if the function is correctly type and add it to the local environment *)    
let tp_fdefn env (Fundefn(FPdecl(tf, _, params), e)) = 
    let envlocal = convert_vardecl_list params @ env.localvar in let te = tp_expr {localvar = envlocal; funbind = env.funbind} e in te = tf;;

let tp_prog (Prog (fdfs, e)) = let initial = environment_initial fdfs in List.for_all (fun fd -> (tp_fdefn initial fd)) fdfs ;;
